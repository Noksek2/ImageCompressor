...
### 스트레치 옵션 (Stretch Option)
- **스트레치(Stretch)** : 스트레치 모드로 이미지 사이즈 변경 시 적용됨.
-- None :  이미지 사이즈가 변경되어도, 비율을 유지하며 이미지를 전부 그린다. 남은 여백은 배경 색상으로 채움.
-- StretchToFit : 이미지의 비율을 바꿔서 결과 이미지에 끼워 맞춘다.
- **배경 색상(BGColor)** : 결과 이미지의 여백 및 배경의 색상을 설정
